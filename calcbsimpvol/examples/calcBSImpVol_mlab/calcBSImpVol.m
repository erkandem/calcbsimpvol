...
P = max(P, 0); % |ed ensure a positive price
...
v1_fixed = fcnv(p,m,n,i,j,x,max(c, 0)); % D- Domain (x<=-1) |ed added max(..., 0)
v2_fixed = fcnv(p,m,n,i,j,-x,max(exp(x).*c + 1 -exp(x), 0)); % Reflection for D+ Domain (x>1)  |ed added max(..., 0)
v = zeros(g,h); v(x(:,:,1)<=0)=v1_fixed(x(:,:,1)<=0); v(x(:,:,1)>0)=v2_fixed(x(:,:,1)>0);
